...
fetch("./examples.json")
    ...
    var parseTree = function (treeIn, $menu) {
        if (treeIn.type === "file") {
            var $item = $("<a>").attr("href", "#").text(treeIn.name).data("path", treeIn.path);
            $menu.append($("<li>").append($item));
            $item.on("click", function (e) { load(treeIn.name, treeIn.path); });
        }
        else {
            var $item = $("<li>").addClass(["dropdown-submenu"]);
            var $a = $("<a>").attr("href", "#").text(treeIn.name).on("click", function (e) { e.stopImmediatePropagation(); });
            var $submenu_1 = $("<ul>").addClass("dropdown-menu");
            $item.append($a, $submenu_1);
            treeIn.children.forEach(function (v) { return parseTree(v, $submenu_1); });
            $menu.append($item);
            $a.dropdown();
        }
    };
    if (tree.children)
        tree.children.forEach(function (v) { return parseTree(v, $menu); });
});
